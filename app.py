...
CLASS_DESC = {
    ...
    "Ranger":    "Wilderness predator with Favored Enemy, Hunter's Mark, and spellcasting (level 1 in 2024 rules). d10 Hit Die.",
...
LOADOUT_DATA = {
    ...
    "Ranger": {
        ...
        "cantrips_count":    0,
        "cantrip_pool":      [],
        "spells_label":      "1st-Level Spells Known — 2024 Rules (choose 2)",
        "spells_count":      2,
...
DM_SYSTEM_PROMPT = """You are a legendary, highly rigorous Dungeon Master executing a solo text D&D campaign built dynamically around the player's custom Character Sheet.
...
STRICT RULE & ABILITY ADHERENCE: You must strictly track and enforce the mechanics of the latest D&D 2024 ruleset. Closely manage character level, active health, environment continuity, spell levels, spell slot limits, resource pools, casting times, ranges, and saving throw DCs for every class feature or spell.
...
def do_custom_item_lookup(item_name: str) -> str:
    ...
    try:
        ...
        headers = {"Content-Type": "application/json"}
        prompt = (
            f"You are a D&D 2024 rules reference. For the item '{item_name}', output ONLY a single "
...
def do_level_up() -> tuple:
    ...
    guide_prompt = (
        ...
        f"Use D&D 2024 rules. Format with clear headers. "
...
def export_markdown() -> str:
    ...
    lines = [
        "# ⚔️ Chronicles of the Forgotten Realm — Adventure Log",
        "",
...
st.markdown('<p class="dnd-subtitle">Solo D&D Text RPG · Powered by Gemini AI · 2024 Rules</p>', unsafe_allow_html=True)
...
with tab_saves:
    ...
    with dl_c1:
        st.download_button(
            ...
            data=save_json,
            file_name=f"dnd_save_{sv_name}.json",
